...
class SpeakRecog:
    ...
    # print(rate)
    engine.setProperty('rate',mycursor.execute('select rate from speech_rate').fetchone()[0])

    """VOLUME"""   
    volume = engine.getProperty('volume')
    engine.setProperty('volume',(mycursor.execute('select vol from volume').fetchone()[0])/10)
    conn.commit()
    conn.close()
...
class SettingWindow:
    ...
    def settingWindow(self,root):
        ...
        self.volume=ttk.Label(self.setting,text='Speech Rate:',borderwidth=0,font=('"Times New Roman"')).place(x=3,y=77)
